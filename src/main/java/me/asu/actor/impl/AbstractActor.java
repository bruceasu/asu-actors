...
public abstract class AbstractActor implements Actor
{
	...
	public boolean receive() {
		...
		if (res) {
			try {
				m.fireMessageListeners(new MessageEvent(this, m, MessageStatus.DELIVERED));
				loopBody(m);
				m.fireMessageListeners(new MessageEvent(this, m, MessageStatus.COMPLETED));
			} catch (Exception e) {
				m.fireMessageListeners(new MessageEvent(this, m, MessageStatus.FAILED));
	...
	protected BlockingQueue<Message> messages = new LinkedBlockingQueue<>(getMaxMessageCount());
